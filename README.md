...
7. 项目产出要求：
   ...
   - 完善 [Casbin-Raft](https://github.com/casbin-rs/casbin-raft) 实现   Rust 版本 [Casbin-Raft](https://github.com/casbin/casbin-raft) ,使用 [Raft-rs](https://github.com/tikv/raft-rs) 框架实现完备的  `Raft` 协议支持，并且支持多种适配器（包括 `diesel-adapter` 、    `sqlx-adapter` ）。要求可以投入实际生产环境
